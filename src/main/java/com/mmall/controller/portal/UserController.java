...
//            session.setAttribute(Const.CURRENT_USER, response.getData()); 单服务器下session存放
            CookieUtil.writeLoginToken(httpServletResponse, session.getId());
            //多服务器下，session存放在redis下
...
//        session.removeAttribute(Const.CURRENT_USER);
    ...
    @ResponseBody
    public ServerResponse<String> restPassword(HttpSession session, String passwordOld, String passwordNew, HttpServletRequest httpServletRequest){
        /*User user = (User) session.getAttribute(Const.CURRENT_USER);*/
        /*不再从session当中获取用户，而是从cookie token当中*/
        String loginToken = CookieUtil.readLoginToken(httpServletRequest);
        /*无法从cookie当中获取到loginToken，直接返回用户未登录*/
